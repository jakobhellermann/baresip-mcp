...
func spawnBaresip(p spawnParams) (*baresipInstance, error) {
	...
	// Proxies that couple media security to signaling security offer RTP/SAVP
	// to devices registered over TLS. Without srtp.so baresip rejects those
	// with the misleading "no common audio or video codecs". It has to be loaded
	// before account.so, which resolves each account's mediaenc as it parses the
	// accounts file and silently drops an encryption it does not know yet.
	srtpModule := optionalModule(modPath, "srtp.so")
	...
	cfg := renderConfig(configParams{
		sipListen:   sipListen,
		modPath:     modPath,
		srtpModule:  srtpModule,
		audioModule: audioModule,
		ctrlPort:    port,
		audioSource: audioSourceLine(greetingPath, useGreeting),
		audioPlayer: audioPlayer,
		ringback:    ringbackLine,
	})
...
// renderConfig builds the baresip config file. Module order is significant:
// account.so resolves each account's mediaenc while it parses the accounts
// file, so every mediaenc provider has to be loaded before it.
func renderConfig(p configParams) string {
	return fmt.Sprintf(`# Generated by baresip-mcp — do not edit.
sip_listen              %s
module_path             %s
module                  g711.so
module                  auconv.so
...
`, p.sipListen, p.modPath, p.srtpModule, p.audioModule, p.ctrlPort, p.audioSource, p.audioPlayer, p.ringback)
}

// optionalModule returns a module line for name if that module is present,
// so a baresip build without it still starts.
func optionalModule(modPath, name string) string {
	if _, err := os.Stat(filepath.Join(modPath, name)); err != nil {
		return ""
	}
	return "module                  " + name
}

// srtpAvailable reports whether the local baresip can do SRTP at all. Setting
// mediaenc without it makes baresip reject the whole account.
func srtpAvailable() bool {
	modPath, err := detectModulePath()
	if err != nil {
		return false
	}
	return optionalModule(modPath, "srtp.so") != ""
}
